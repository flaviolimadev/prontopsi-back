...
export class PacientesController {
  ...
  async listPacienteFiles(@Request() req, @Param('id') id: string) {
    try {
      const userId = req.user.sub;
      console.log(`Listando arquivos para paciente ${id} e usuário ${userId}`);
      
      // Usar query builder para evitar problemas com relacionamentos
      const arquivos = await this.pacienteArquivoRepo
        .createQueryBuilder('arquivo')
        .where('arquivo.paciente_id = :pacienteId', { pacienteId: id })
        .andWhere('arquivo.user_id = :userId', { userId })
        .orderBy('arquivo.created_at', 'DESC')
        .getMany();
      
      console.log(`Encontrados ${arquivos.length} arquivos`);
      return arquivos;
    } catch (error) {
      console.error('Erro ao listar arquivos do paciente:', error);
