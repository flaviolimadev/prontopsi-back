...
export class CloudflareR2Service {
  ...
  constructor(private configService: ConfigService) {
    ...
    this.r2Enabled = !!(accountId && accessKeyId && secretAccessKey && bucketName);

    if (!this.r2Enabled) {
      this.logger.warn('⚠️ Serviço Cloudflare R2 desabilitado - Configurações não encontradas');
      return;
    }
  ...
  async uploadFile(
    ...
    if (!this.r2Enabled || !this.s3Client) {
      throw new Error('Serviço Cloudflare R2 não está configurado');
    }
  ...
  async deleteFile(key: string): Promise<boolean> {
    if (!this.r2Enabled || !this.s3Client) {
      this.logger.warn('Tentativa de deletar arquivo com R2 desabilitado');
      return false;
    }
