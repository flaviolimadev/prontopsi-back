...
export class User {
  ...
  // Campos de plano e trial
  @Column({ name: 'plan_type', type: 'varchar', length: 20, default: 'gratuito' })
  planType: string;

  @Column({ name: 'subscription_status', type: 'varchar', length: 20, default: 'active' })
  ...
  @Column({ name: 'trial_used', type: 'boolean', default: false })
  trialUsed: boolean;
  ...
  // Métodos auxiliares para trial
  isOnTrial(): boolean {
    if (!this.trialEndsAt || this.subscriptionStatus !== 'trial') {
      return false;
    }
  ...
  getTrialDaysRemaining(): number {
    ...
    const now = new Date();
    const endDate = new Date(this.trialEndsAt);
    const diff = endDate.getTime() - now.getTime();
    return Math.max(0, Math.ceil(diff / (1000 * 60 * 60 * 24)));
  }

  canStartTrial(): boolean {
    return !this.trialUsed && this.planType === 'gratuito';
  }
} 