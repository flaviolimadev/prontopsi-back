...
export class PagamentosService {
  ...
  async create(userId: string, createPagamentoDto: CreatePagamentoDto): Promise<PagamentoResponseDto> {
    const pagamento = this.pagamentoRepository.create({
      ...
      metodoPagamento: createPagamentoDto.metodoPagamento || 'PIX',
    });
  ...
  async findAll(
    ...
    if (search) {
      queryBuilder.andWhere(
        '(pagamento.descricao ILIKE :search OR paciente.nome ILIKE :search)',
        { search: `%${search}%` },
      );
    }
  ...
  private toResponseDto(pagamento: Pagamento): PagamentoResponseDto {
    ...
    return {
      ...
      pacote: pagamento.pacote ? {
        ...
        nome: pagamento.pacote.nome,
        valor_total: parseFloat(pagamento.pacote.valorTotal?.toString() || '0'),
      } : null,
